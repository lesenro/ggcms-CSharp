<template>
  <div class="page-panel" v-loading="loading">
    ...
    <el-dialog title="广告信息" :visible.sync="dialogFormVisible" @open="dialogOpened">
...
export default {
  ...
  async created() {
    ...
    upload.controlProps.httpRequest = ev => this.onFileSelect(ev, "Image");
    this.formSettings = settings;
    let grps = await this.getDictList({
      QueryString: 'GroupKey=="ads_group" and DictType==1 and DictStatus=1',
      PageSize: 0,
      OrderBy: "OrderId asc"
    });
    if (grps.Records.length > 0) {
      this.adv_groups = grps.Records.map(x => {
        return {
          label: x.DictName,
          value: x.DictKey
        };
  ...
  methods: {
    ...
    ...mapActions("advert", ["getList", "save", "del", "getById"]),
    ...
    dialogOpened() {
      ...
      form.setOptions("GroupKey", this.adv_groups);
