...
import { PageData, AppService, AdminService } from "app/services";
...
export class ModulesComponent implements OnInit {
  ...
  itemDelete() {
    ...
    this.appServ.MessageBox("是否删除：" + ids.length + " 条记录", "删除提醒",
      {
        accept: () => {
          this.adminServ.DelModules(ids).then(data => {
            if (data.Code == 0) {
  ...
  pageChange(ev) {
    this.appServ.goRouter('/index/modules', { page: ev });
  }
  dataLoad() {
    this.adminServ.GetModulesList(this.pdata.pagenum).then(data => {
      if (data.Code == 0) {
